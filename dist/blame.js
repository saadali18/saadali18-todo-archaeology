...
const UNKNOWN = { author: "unknown", email: "unknown", date: null, commit: null, ageDays: null };
function runGitBlame(cwd, file, line) {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || `git blame exited with ${code}`));
        });
    });
}
export function parseBlamePorcelain(output) {
    ...
    if (!header)
        return null;
    const commit = header.split(" ")[0];
    if (!commit || /^0{40}$/.test(commit))
        return null; // uncommitted / not found
    let author = "unknown";
    let email = "unknown";
    let timestamp = null;
...
export async function blameItem(cwd, item, now) {
    try {
...
export async function blameAll(items, options) {
