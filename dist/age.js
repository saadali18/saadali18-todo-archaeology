...
export function computeAgeDays(fromDate, now = new Date()) {
...
export function humanizeAgeLong(days) {
    if (days < 30)
        return pluralize(days, "day");
    if (days < 365)
        return pluralize(Math.max(1, Math.round(days / DAYS_PER_MONTH)), "month");
    const years = Math.floor(days / DAYS_PER_YEAR);
    const months = Math.round((days - years * DAYS_PER_YEAR) / DAYS_PER_MONTH);
    if (months <= 0)
        return pluralize(years, "year");
    if (months >= 12)
        return pluralize(years + 1, "year");
    return `${pluralize(years, "year")}, ${pluralize(months, "month")}`;
}
/** "6y 3m" / "11m" / "3d" */
export function humanizeAgeShort(days) {
    if (days < 30)
        return `${days}d`;
    if (days < 365)
        return `${Math.max(1, Math.round(days / DAYS_PER_MONTH))}m`;
    const years = Math.floor(days / DAYS_PER_YEAR);
    const months = Math.round((days - years * DAYS_PER_YEAR) / DAYS_PER_MONTH);
    if (months <= 0)
        return `${years}y`;
